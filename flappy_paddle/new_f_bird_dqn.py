...
def evaluate(ple_env, agent, render=False):
    ...
    for i in range(5):
        ...
        while True:
            ...
            # if render:
            #     frame = ple_env.getScreenRGB()
            #     frame = cv2.transpose(frame)
            #     score = ple_env.score()
            #     font = cv2.FONT_HERSHEY_SCRIPT_COMPLEX
            #     frame = cv2.putText(frame, str(int(score)), (25, 25), font, 1.2, (255, 255, 255), 2)
            #     cv2.imshow("flappy paddle", frame)
            #     cv2.waitKey(10)
...
def main():
    # 创建环境
    game = FlappyPaddle(pipe_gap=140)
    ...
    # 加载模型
    if os.path.exists('./model_dqn_578.8.ckpt'):
        agent.restore('./model_dqn_578.8.ckpt')
    ...
    while episode < max_episode:  # 训练max_episode个回合，test部分不计算入episode数量
        ...
        if eval_reward > 270:
            agent.save(f'./model_dqn_{str(eval_reward)}.ckpt')

    # 保存模型到文件 ./model.ckpt
    agent.save('./model_dqn_paddle.ckpt')
